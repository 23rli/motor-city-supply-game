...
export function RecipePanel({ config }: RecipePanelProps) {
  return (
    <div className="recipe-table" role="table" aria-label="Car recipes and economics">
      <div className="recipe-row recipe-head" role="row">
        ...
        <span role="columnheader">WIP</span>
      </div>
      {CAR_MODELS.map((model) => (
        <div className="recipe-row" role="row" key={model}>
          ...
          <span role="cell">-${config.wipPenalty[model].toFixed(2)}</span>
        </div>
      ))}
    </div>
  )
